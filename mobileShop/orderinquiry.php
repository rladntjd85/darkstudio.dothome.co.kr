...
if ($is_member)
{
    $sql_common = " from g5_shop_order2 where mb_id = '{$member['mb_id']}' ";
}
else if ($od_id && $od_pwd) // 비회원인 경우 주문서번호와 비밀번호가 넘어왔다면
{
    $sql_common = " from g5_shop_order2 where od_id = '$od_id' and od_pwd = '$od_pwd' ";
}
...
if (!$is_member)
{
    $sql = " select od_id, od_time, od_ip from g5_shop_order2 where od_id = '$od_id' and od_pwd = '$od_pwd' ";
    $row = sql_fetch($sql);
    if ($row['od_id']) {
